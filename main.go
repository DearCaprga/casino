...
func main() {
	...
	// Запуск фоновой задачи для проверки таймаутов игр
	go checkGameTimeouts()
...
// Фоновая задача для проверки таймаутов игр
func checkGameTimeouts() {
	for {
		...
		for playerID, game := range games {
			if !game.Finished && game.Started {
				timeElapsed := now.Sub(game.StartTime).Seconds()
				if int(timeElapsed) > game.TimeLimit {
...
// Обработчики игроков (остаются без изменений, кроме createPlayerHandler)
...
// getPlayersHandler возвращает список всех игроков
